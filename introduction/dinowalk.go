...
func run() error {
	...
	for running {
		...
		rect := sdl.Rect{x, y, 680, 472}
		if shouldFlip {
			renderer.CopyEx(dinoSprites[sprite%10], nil, &rect, 0.0, nil, sdl.FLIP_HORIZONTAL)
		} else {
			renderer.CopyEx(dinoSprites[sprite%10], nil, &rect, 0.0, nil, sdl.FLIP_NONE)
		}

		renderer.Present()
		sdl.Delay(70)
		sprite++
		if x > 1000-(680/4) || !isGoingRight {
			isGoingRight = false
			shouldFlip = true
			x -= vel
		}
